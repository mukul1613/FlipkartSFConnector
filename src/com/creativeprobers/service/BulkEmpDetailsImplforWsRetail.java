...
import org.tempuri.EmpLocDetails;
...
public class BulkEmpDetailsImplforWsRetail implements Job {
	private static Logger logger = Logger
			.getLogger(BulkEmpDetailsImplforWsRetail.class);
	...
	private HashMap<String, EmpLocDetails> empDetailsMapList = new HashMap<String, EmpLocDetails>();
	private HashMap<String, PicklistOption> empEmploymentStatusMapList = new HashMap<String, PicklistOption>();

	public org.tempuri.EmpLocDetails[] getBulkEmpDetails(
			java.lang.String APPID, java.lang.String empType)
			throws RemoteException {

		EmpLocDetails[] empDetailsList = getBulkEmpDetails_from_csv(APPID,
				empType);
	...
	private EmpLocDetails[] getBulkEmpDetails_For_Caching(
			java.lang.String empType) throws RemoteException {
		EmpLocDetails[] empDetailsList = null;
		try {
			...
			for (EmpLocDetails empDetails : empDetailsList) {
	...
	private EmpLocDetails[] getBulkEmployeeDetails(String empType)
			throws RemoteException, SFWebServiceFaultException {

		EmpLocDetails empDetails = new EmpLocDetails();

		EmpLocDetails[] empDetailsList = null;
		ArrayList<EmpLocDetails> empDetailsArrayList = new ArrayList<EmpLocDetails>();
		...
		if (isValidLogin) {
			try {
				...
				while (qResult.isHasMore()) {
					...
					logger.debug("Inside2 qResult:: " + qResult);
					srObjectarr = qResult.getSfobject();
					logger.debug("srObjectarr ::" + srObjectarr);
					if (srObjectarr != null) {
						logger.debug("srObjectarr::" + srObjectarr);
						if (srObjectarr.length > 0) {
							for (int i = 0; i < srObjectarr.length; i++) {
				...
				empDetailsList = new EmpLocDetails[empDetailsArrayList.size()];
	...
	private EmpLocDetails getQueryResultForBulkEmpDetails(
			SFObject[] srObjectarr, int i) throws Exception {

		logger.debug("getQueryResultForEmpDetails Method");
		EmpLocDetails empDetails = new EmpLocDetails();
		EmpLocDetails managerDetails = new EmpLocDetails();
		EmpLocDetails managerMangersDetails = new EmpLocDetails();
		...
		managerMangersDetails = initializeEmpLocDetails(managerMangersDetails);
		...
		if (isValidLogin) {
			try {
				...
				if (srObjectarr != null) {
					...
					logger.info(empEmploymentStatusMapList.get(
	...
	private EmpLocDetails getQueryResultForEmpDetailsMap(
			SFObject[] srObjectarr, int i) throws Exception {
	...
	private boolean createCSV(
			org.tempuri.EmpLocDetails[] empDetailsList_active,
			org.tempuri.EmpLocDetails[] empDetailsList_inactive, File file)
			throws Exception {
		/*
		 * BulkEmpDetailsImpl bulkEmpDetailsImpl=new BulkEmpDetailsImpl();
		 * bulkEmpDetailsImpl.getBulkEmpDetails("", "");
		 */
		...
		try {
			if (empDetailsList_active != null
				...
				for (int i = 0; i < empDetailsList_active.length; i++) {
					...
					// csvOutput.write(empDetailsList[i].getPassportvalidity());
	...
	private EmpLocDetails[] readCSVData(java.lang.String APPID,
		...
		try {
			...
			while (reader.readRecord()) {
				...
				if (Location.contains("(")) {
					...
					String loc1 = Location.substring(Location.indexOf('('));
		...
		// EmpDetails[][] empDetailsList;
		EmpLocDetails[] empDetailsList = new EmpLocDetails[empDetailsArrayList
				.size()];
		for (int i = 0; i < empDetailsArrayList.size(); i++) {
			empDetailsList[i] = empDetailsArrayList.get(i);
		}
	...
	public static void main(String[] args) throws Exception {
		/*
		 * BulkEmpDetailsImplforWsRetail bulkEmpDetailsImpl = new
		 * BulkEmpDetailsImplforWsRetail();
	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		...
		try {
			...
			EmpLocDetails[] empDetailsList_active = getBulkEmpDetails_For_Caching("active");
			EmpLocDetails[] empDetailsList_inactive = getBulkEmpDetails_For_Caching("inactive");
	...
	public EmpLocDetails[] getBulkEmpDetails_from_csv(java.lang.String APPID,
			java.lang.String empType) throws RemoteException {
		...
		// File file = new
		// File("C:/Users/Rajasekhar Mallam/Desktop/SF/FilpKart/Details.csv");
		EmpLocDetails[] empDetailsList = bulkEmpDetailsImpl.readCSVData(APPID,
				empType, file);
